...
namespace SimpletextingAPI
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var listIds = configuration.GetSection("ListIds").Get<List<string>>();
            ...
            if(listIds == null || !listIds.Any()) {
                Console.WriteLine("Must specify at least one list name or Id");
                return;
            }
            ...
            // New users, or users with changed names and emails
            List<User> usersToUpsert = adUsers
                .Where(ad => !apiUsers.Any(api => api.ContactPhone == ad.ContactPhone) ||
                         ...
                         (api.FirstName != ad.FirstName || api.LastName != ad.LastName || api.Email != ad.Email))
                      ).ToList();
